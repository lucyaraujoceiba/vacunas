...
public class ServicioCrearUsuarioTest {
    ...
    void deberiaLanzarUnaExepcionCuandoSeValideLaExistenciaDelUsuario() {
        ...
        Mockito.when(repositorioUsuario.existe(Mockito.anyString(),Mockito.anyString())).thenReturn(true);
    ...
    void deberiaCrearElUsuarioDeManeraCorrecta() {
        ...
        Usuario usuario = new UsuarioTestDataBuilder().build();
        RepositorioUsuario repositorioUsuario = Mockito.mock(RepositorioUsuario.class);
        Mockito.when(repositorioUsuario.existe(Mockito.anyString(),Mockito.anyString())).thenReturn(false);
